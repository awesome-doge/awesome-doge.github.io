...
## 本地server
當我們製作好，文件夾好，其實jekyll同時也支持伺服器。你衹要在根目錄底下執行`jekyll serv` 你就會在本地端啟動一個伺服器出來。這個伺服器是HTTP的伺服器，同時它也會開啟一個4000端口出來。所以你這時開啟一個瀏覽器輸入`http://127.0.0.1:4000` 或是 `http://localhost:4000` 就可以順利的造訪這個網頁。
